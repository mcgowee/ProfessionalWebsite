...
function List($$payload, $$props) {
  ...
  function getListItemIndex(element22) {
    const nearestParent = closest(element22, ".mdc-deprecated-list-item, .mdc-deprecated-list");
...
function Item($$payload, $$props) {
  ...
  function getPrimaryText() {
    const element22 = getElement();
    const primaryText = element22.querySelector(".mdc-deprecated-list-item__primary-text");
    if (primaryText) {
      return primaryText.textContent ?? "";
    }
    const text = element22.querySelector(".mdc-deprecated-list-item__text");
    if (text) {
      return text.textContent ?? "";
    }
    return element22.textContent ?? "";
  }
  function getElement() {
    return element2.getElement();
  }
...
function Button($$payload, $$props) {
  ...
  const actionProp = context === "dialog:action" && action != null ? { "data-mdc-dialog-action": action } : { action };
  const defaultProp = context === "dialog:action" && defaultAction ? { "data-mdc-dialog-button-default": "" } : {};
  const secondaryProp = context === "banner" ? {} : { secondary };
  restProps.disabled;
  ...
  MyComponent($$payload, spread_props([
    ...
    restProps,
    {
      ...
      children: ($$payload2) => {
        $$payload2.out += `<div class="mdc-button__ripple"></div> `;
        children?.($$payload2);
        $$payload2.out += `<!---->`;
        if (touch) {
          $$payload2.out += "<!--[-->";
          $$payload2.out += `<div class="mdc-button__touch"></div>`;
        } else {
          $$payload2.out += "<!--[!-->";
        }
        $$payload2.out += `<!--]-->`;
